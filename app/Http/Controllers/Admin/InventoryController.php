...
class InventoryController extends Controller
{
    ...
    {
        // Runs on every Inventory page load/refresh and every live poll —
        // this hosting has no cron/scheduler, so real Inventory traffic is
        // what drives automatic low-stock draft PO creation. Isolated in
        // its own try/catch so a failure here can never break the Inventory
        // page itself; see PurchaseOrderController::autoGenerateDraftPurchaseOrders().
        try {
            PurchaseOrderController::autoGenerateDraftPurchaseOrders();
        } catch (Throwable $e) {
            Log::error('Automatic low-stock PO generation failed: '.$e->getMessage());
        }
        ...
        // REQ044: search by name or keyword (name + model + barcode + category)
        if ($search) {
