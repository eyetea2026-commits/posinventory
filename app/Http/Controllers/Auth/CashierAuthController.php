...
class CashierAuthController extends Controller
{
    ...
    public function refunds(Request $request)
    {
        ...
        $refunds = \App\Models\SalesReturn::with(['staff', 'salesTransaction'])
        ...
        return view('cashier.refunds', compact('refunds', 'search'));
    }
    ...
    /**
     * Create a new refund request (REQ103-105)
     */
    public function createRefund(Request $request)
    {
        ...
        // A transaction can only be refunded up to the quantity actually sold —
        // without this, repeated refund requests for the same line item could
        // each get approved and inflate inventory/payouts beyond what was sold.
        $alreadyRequested = \App\Models\SalesReturn::where('SalesTransactionID', $data['transaction_id'])
            ->where('ProductID', $data['product_id'])
        ...
        if ($alreadyRequested + $data['quantity'] > $salesItem->Quantity) {
            return response()->json([
                'success' => false,
                'message' => 'Refund quantity exceeds the quantity sold for this product.',
            ], 400);
        }

        $unitPrice = $salesItem->UnitPrice;
        $refundAmount = $unitPrice * $data['quantity'];
    ...
    /**
     * Get transaction details for refund (REQ104 - Retrieve Transaction)
     */
    public function getTransactionDetails($transactionId)
    {
        ...
        return response()->json([
            'success' => true,
            'transaction' => [
                'id' => $transaction->SalesTransactionID,
                'date' => $transaction->SalesTransactionDate,
                'customer_name' => $transaction->CustomerName,
                'items' => $items,
                'billing' => [
                    'amount' => $transaction->billing?->BillingAmount ?? 0,
                ],
            ],
        ]);
    }
    ...
    /**
     * Process a refund after approval (REQ107-109)
     */
    public function processRefund(Request $request, $refundId)
    {
        ...
        $salesReturn = \App\Models\SalesReturn::find($refundId);
        ...
        if ($salesReturn->Status !== 'approved') {
            return response()->json(['success' => false, 'message' => 'Refund must be approved before processing.'], 400);
        }
    ...
    /**
     * Get refund details (REQ106)
     */
    public function getRefundDetails($refundId)
    {
        $refund = \App\Models\SalesReturn::with(['transaction', 'product'])
    ...
    /**
     * Get cashier stats (REQ091 - Total Refunds Handled)
     */
    public function getCashierStats()
    {
        ...
        $totalRefunds = \App\Models\SalesReturn::where('StaffID', $staff->StaffID)->count();
        $pendingRefunds = \App\Models\SalesReturn::where('StaffID', $staff->StaffID)->where('Status', 'pending')->count();
        $approvedRefunds = \App\Models\SalesReturn::where('StaffID', $staff->StaffID)->where('Status', 'approved')->count();
        $rejectedRefunds = \App\Models\SalesReturn::where('StaffID', $staff->StaffID)->where('Status', 'rejected')->count();
