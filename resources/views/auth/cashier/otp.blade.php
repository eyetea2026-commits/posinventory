...
<head>
    ...
    <title>Cashier OTP Verification - CCTV Express TACURONG</title>
...
<body>
    <div class="login-background">
        <section class="login-card">
            ...
            <form method="POST" action="{{ route('cashier.otp.verify') }}">
                @csrf
                ...
                <div class="login-footer">
                    <a class="small-link" href="{{ route('cashier.forgot') }}">&#8592; Back to Forgot Password</a>
                </div>
            </form>

            <div style="margin-top: 20px; text-align: center;">
                <p style="color: #94a3b8; font-size: 0.9rem; margin-bottom: 12px;">Didn't receive the code?</p>
                <form method="POST" action="{{ route('cashier.forgot.post') }}" style="display: inline;">
                    @csrf
                    <input type="hidden" name="email" value="{{ $email }}" />
